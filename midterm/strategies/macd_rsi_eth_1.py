...
def strat(data):
    ...
    for value in data["Signal"]:
        if value == prev:
            ...
            #elif (value==1 and prev==-1) or (value==-1 or prev==1):
            #    prev = 0
            #    signal.append()
            #    trade_type.append('close')                
            #elif value == 1 and prev == -1:
            #    signal.append(2)
            #    trade_type.append('long_reversal')
            #elif value==-1 and prev==1:
            #    signal.append(-2)
            #    trade_type.append('short_reversal')
            #else:
            #    signal.append(0)
            #    trade_type.append('')
        else:
...
def main():
     data = pd.read_csv("ETH_DATA/ETHUSDT_30m.csv")

     processed_data = process_data(data)

     result_data = strat(processed_data)

     csv_file_path = "ETHresults_1.csv"
